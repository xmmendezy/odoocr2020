odoo.define('custom_studio.WebClient', function (require) {
"use strict";
...
const studioBus = require('custom_studio.bus');
const SystrayItem = require('custom_studio.SystrayItem');
...
WebClient.include({
    ...
    openAppCreator: async function () {
        await this.do_action('action_custom_studio_app_creator');
    ...
    toggleHomeMenu: function (display) {
        if (this.studioMode) {
            if (display) {
                ...
                if (action && action.tag === 'action_custom_studio_app_creator') {
    ...
    _closeStudio: async function () {
        ...
        } else if (action.tag === 'action_custom_studio_app_creator') {
     ...
     * @returns {Promise}
     */
    _studioAssets: ['web_editor.compiled_assets_wysiwyg', 'custom_studio.compiled_assets_studio'],
    ...
    _openStudioMain: function (options) {
        return this.do_action('action_custom_studio_action_editor', options);
    },
    /**
     * @private
     * @returns {Promise}
