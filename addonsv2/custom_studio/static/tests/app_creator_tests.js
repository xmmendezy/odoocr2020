odoo.define('custom_studio.AppCreator_tests', function (require) {
    "use strict";

    const AppCreatorWrapper = require('custom_studio.AppCreator');
    ...
    QUnit.module('Studio', {
        ...
        QUnit.test('app creator: standard flow with model creation', async function (assert) {
            ...
            const { wrapper, appCreator } = await createAppCreator({
                ...
                rpc: async (route, params) => {
                    if (route === '/custom_studio/create_new_app') {
            ...
            assert.containsNone(appCreator, '.o_custom_studio_app_creator_previous',
                "Previous button should not be rendered at step welcome");
            assert.hasClass(appCreator.el.querySelector('.o_custom_studio_app_creator_next'), 'is_ready',
                "Next button should be ready at step welcome");

            // go to step: 'app'
            await testUtils.dom.click(appCreator.el.querySelector('.o_custom_studio_app_creator_next'));

            assert.strictEqual(appCreator.state.step, 'app',
                "Current step should be app");
            assert.containsOnce(appCreator, '.o_custom_studio_icon_creator .o_custom_studio_selectors',
            ...
            await testUtils.dom.click(appCreator.el.getElementsByClassName('o_custom_studio_selector')[0]);

            assert.containsOnce(appCreator, '.o_custom_studio_palette',
                "the first palette should be open");

            await testUtils.dom.triggerEvent(appCreator.el.querySelector('.o_custom_studio_palette'), 'mouseleave');

            assert.containsNone(appCreator, '.o_custom_studio_palette',
                "leaving palette with mouse should close it");

            await testUtils.dom.click(appCreator.el.querySelectorAll('.o_custom_studio_selectors > .o_custom_studio_selector')[0]);
            await testUtils.dom.click(appCreator.el.querySelectorAll('.o_custom_studio_selectors > .o_custom_studio_selector')[1]);

            assert.containsOnce(appCreator, '.o_custom_studio_palette',
                "opening another palette should close the first");

            await testUtils.dom.click(appCreator.el.querySelectorAll('.o_custom_studio_palette div')[2]);
            await testUtils.dom.click(appCreator.el.querySelectorAll('.o_custom_studio_selectors > .o_custom_studio_selector')[2]);
            await testUtils.dom.click(appCreator.el.querySelectorAll('.o_custom_studio_icons_library div')[43]);

            await testUtils.dom.triggerEvent(appCreator.el.querySelector('.o_custom_studio_icons_library'), 'mouseleave');

            assert.containsNone(appCreator, '.o_custom_studio_palette',
                "no palette should be visible anymore");

            assert.strictEqual(appCreator.el.querySelectorAll('.o_custom_studio_selector')[1].style.backgroundColor, 'rgb(0, 222, 201)', // translation of #00dec9
                "color selector should have changed");
            assert.strictEqual(icon.style.color, 'rgb(0, 222, 201)',
                "icon color should also have changed");

            assert.hasClass(appCreator.el.querySelector('.o_custom_studio_selector i'), 'fa fa-heart',
            ...
            await testUtils.dom.triggerEvent(appCreator.el.querySelector('.o_custom_studio_upload input'), 'change');

            assert.strictEqual(appCreator.state.iconData.uploaded_attachment_id, 666,
                "attachment id should have been given by the RPC");
            assert.strictEqual(appCreator.el.querySelector('.o_custom_studio_uploaded_image').style.backgroundImage,
            ...
            assert.hasClass(appNameInput, 'o_custom_studio_app_creator_field_warning',
                "Input should be in warning mode");

            await testUtils.fields.editInput(appCreator.el.querySelector('input[name="appName"]'), "Kikou");
            assert.doesNotHaveClass(appNameInput, 'o_custom_studio_app_creator_field_warning',
            ...
            assert.containsNone(appCreator, '.o_custom_studio_selectors',
                "Icon creator should be rendered in readonly mode");

            // try to go to next step
            await testUtils.dom.click(appCreator.el.querySelector('.o_custom_studio_app_creator_next'));

            assert.hasClass(appCreator.el.querySelector('input[name="menuName"]').parentNode, 'o_custom_studio_app_creator_field_warning',
                "Input should be in warning mode");
            ...
            await testUtils.dom.click(appCreator.el.querySelector('.o_custom_studio_model_configurator_previous'));
            await testUtils.dom.click(appCreator.el.querySelector('.o_custom_studio_app_creator_next'));
            ...
            await testUtils.dom.click(appCreator.el.querySelector('.o_custom_studio_model_configurator_next'));
        ...
        QUnit.test('app creator: debug flow with existing model', async function (assert) {
                ...
                async rpc(route, params) {
                    ...
                    switch (route) {
                        case '/web/dataset/call_kw/ir.model/name_search':
                            ...
                            return [[69, "The Value"]];
                        case '/custom_studio/create_new_app':
            ...
            let buttonNext = appCreator.el.querySelector('button.o_custom_studio_app_creator_next');
            ...
            buttonNext = appCreator.el.querySelector('button.o_custom_studio_app_creator_next');
            ...
            assert.doesNotHaveClass(appCreator.el.querySelector('.o_custom_studio_app_creator_model'),
                'o_custom_studio_app_creator_field_warning');
            ...
            assert.hasClass(appCreator.el.querySelector('.o_custom_studio_app_creator_model'),
                'o_custom_studio_app_creator_field_warning');
            assert.doesNotHaveClass(buttonNext, 'is_ready');
            ...
            assert.verifySteps(['/web/dataset/call_kw/ir.model/name_search', '/custom_studio/create_new_app']);
        ...
        QUnit.test('icon creator: with initial web icon data', async function (assert) {
            ...
            assert.strictEqual(parent.el.querySelector('.o_custom_studio_uploaded_image').style.backgroundImage,
                `url(\"${sampleIconUrl}\")`,
                "displayed image should prioritize web icon data");

            // click on first link: "Design icon"
            await testUtils.dom.click(parent.el.querySelector('.o_custom_studio_upload a'));
