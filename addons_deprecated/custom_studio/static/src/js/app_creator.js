odoo.define('custom_studio.AppCreator', function (require) {
    "use strict";

    const AbstractAction = require('web.AbstractAction');
    const { action_registry } = require('web.core');
    const { COLORS, BG_COLORS, ICONS } = require('custom_studio.utils');
    const { FieldMany2One } = require('web.relational_fields');
    const IconCreator = require('custom_studio.IconCreator');
    const { ModelConfigurator } = require('custom_studio.ModelConfigurator');
    ...
    class AppCreator extends Component {
        ...
        async _createNewApp() {
            ...
            const result = await this.rpc({
                route: '/custom_studio/create_new_app',
                params: {
        ...
        _onKeydown(ev) {
            if (
                ev.key === 'Enter' && !(
                    ev.target.classList &&
                    ev.target.classList.contains('o_custom_studio_app_creator_previous')
                )
            ) {
                ev.preventDefault();
                this._onNext();
    ...
    action_registry.add('action_custom_studio_app_creator', AppCreatorWrapper);
