odoo.define('custom_studio.NewViewDialog', function (require) {
"use strict";
...
var NewViewDialog = Dialog.extend({
    template: 'custom_studio.NewViewDialog',
    /**
     * @constructor
     * @param {Object} params
    ...
    start: function () {
        ...
        this.$modal.addClass('o_custom_studio_new_view_modal');
    },
    ...
    _onSave: function () {
        var self = this;
        var attrs = {};
        $.each(this.$('.o_custom_studio_select select'), function (key, select) {
            attrs[$(select).data('field')] = $(select).val();
        });
        ajax.jsonRpc('/custom_studio/create_default_view', 'call', {
            model: this.model,
