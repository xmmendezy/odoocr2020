odoo.define('custom_studio.ViewEditorManager', function (require) {
"use strict";
...
var AbstractEditorManager = require('custom_studio.AbstractEditorManager');
var bus = require('custom_studio.bus');
var EditorMixin = require('custom_studio.EditorMixin');
var EditorMixinOwl = require('custom_studio.EditorMixinOwl');

var CalendarEditor = require('custom_studio.CalendarEditor');
var FormEditor = require('custom_studio.FormEditor');
var GraphEditor = require('custom_studio.GraphEditor');
var KanbanEditor = require('custom_studio.KanbanEditor');
var ListEditor = require('custom_studio.ListEditor');
var SearchEditor = require('custom_studio.SearchEditor');
var SearchRenderer = require('custom_studio.SearchRenderer');

var FieldSelectorDialog = require('custom_studio.FieldSelectorDialog');
var NewButtonBoxDialog = require('custom_studio.NewButtonBoxDialog');
var NewFieldDialog = require('custom_studio.NewFieldDialog');
var utils = require('custom_studio.utils');
var ViewEditorSidebar = require('custom_studio.ViewEditorSidebar');
...
var ViewEditorManager = AbstractEditorManager.extend({
    ...
    updateEditor: function (options) {
        ...
        return this._instantiateEditor(options).then(function (editor) {
            ...
            return prom.then(function () {
                dom.append(self.$('.o_custom_studio_view_renderer'), [fragment], {
                    in_DOM: self.isInDOM,
                ...
                def_field_values = new Promise(function (resolve, reject) {
                    ...
                    prom.then(function (openFieldDialog) {
                        if (!openFieldDialog) {
                            ...
                            var $message = $(QWeb.render('custom_studio.FieldOne2manyWarning'));
    ...
    _applyChangeHandling: function (result, opID) {
        ...
        // studio_view_id must be updated (but /custom_studio/edit_view_arch
    ...
    _createInlineView: function (type, field_name) {
        ...
        var prom = this._rpc({
            route: '/custom_studio/create_inline_view',
            params: {
                model: this.x2mModel,
                view_id: this.view_id,
    ...
    _editField(modelName, fieldName, values, forceEdit) {
        return this._rpc({
            route: '/custom_studio/edit_field',
            params: {
                model_name: modelName,
    ...
    _editViewArch: function (view_id, view_arch) {
        core.bus.trigger('clear_cache');
        return this._rpc({
            route: '/custom_studio/edit_view_arch',
            params: {
                view_id: view_id,
                view_arch: view_arch,
                // We write views in the base language to make sure we do it on the source term field
    ...
    _getDefaultValue: function (model_name, field_name) {
        return this._rpc({
            route: '/custom_studio/get_default_value',
            params: {
    ...
    _getEmailAlias: function (model_name) {
        return this._rpc({
            route: '/custom_studio/get_email_alias',
            params: {
                model_name: model_name,
            },
        });
    ...
    _renameField: function (oldName, newName) {
        ...
        return this._rpc({
            route: '/custom_studio/rename_field',
            params: {
    ...
    _restoreDefaultView: async function (view_id) {
        ...
        const result = await this._rpc({
            route: '/custom_studio/restore_default_view',
            params: {
                view_id: view_id,
            },
        });
    ...
    _setDefaultValue: function (model_name, field_name, value) {
        ...
        return this._rpc({route: '/custom_studio/set_default_value', params: params});
    },
    /**
     * @private
     * @param {String} model_name
    ...
    _setEmailAlias: function (model_name, value) {
        return this._rpc({
            route: '/custom_studio/set_email_alias',
            params: {
                model_name: model_name,
                value: value,
            },
    ...
    */
    ...
    _onApprovalChange: async function(ev) {
        ...
        bus.trigger('toggle_snack_bar', 'saving');
        const result = await this._rpc({
            route: '/custom_studio/edit_approval',
            params: {
                model: this.model_name,
            ...
            self._editField(modelName, field.name, values).then(function (result) {
                ...
                if (result && result.records_linked) {
                    ...
                    Dialog.confirm(self, message, {
                        ...
                        dialogClass: 'o_custom_studio_preserve_space'
                    });
                } else {
                    _closeDialog();
                }
    ...
    _onNodeClicked: function (ev) {
        ...
        if (this.view_type === 'form' && node.tag === 'field') {
            ...
            if (isX2Many && !_.contains(notEditableWidgets, attrs.widget)) {
                ...
                // create the subviews
                var message = $(QWeb.render('custom_studio.X2ManyEdit'));
                var options = {
                ...
                $node.find('.o_custom_studio_editX2Many').click(function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                    self._openX2mEditor(
                        node.attrs.name,
