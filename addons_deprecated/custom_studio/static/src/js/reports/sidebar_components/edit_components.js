odoo.define('custom_studio.reportEditComponents', function (require) {
"use strict";
...
var Abstract = require('custom_studio.AbstractReportComponent');
...
var LayoutEditable = AbstractEditComponent.extend({
    name: 'layout',
    template : 'custom_studio.ReportLayoutEditable',
    events : _.extend({}, AbstractEditComponent.prototype.events, {
        "change .o_custom_studio_margin>input": "_onMarginInputChange",
        "change .o_custom_studio_width>input": "_onWidthInputChange",
        "click .o_custom_studio_font_size .dropdown-item-text": "_onFontSizeChange",
        "change .o_custom_studio_table_style > select": "_onTableStyleInputChange",
        "click .o_custom_studio_text_decoration button": "_onTextDecorationChange",
        "click .o_custom_studio_text_alignment button": "_onTextAlignmentChange",
        "change .o_custom_studio_classes>input": "_onClassesChange",
        "click .o_custom_studio_colors .o_custom_studio_reset_color": "_onResetColor",
        "click .o_custom_studio_colors .o_custom_studio_custom_color": "_onCustomColor",
    }),
    /**
     * @override
     */
    ...
    renderElement: function() {
        var self = this;
        this._super.apply(this, arguments);
        this.$('.o_custom_studio_background_colorpicker .o_custom_studio_color_palette').append(this._createPalette());
        this.$('.o_custom_studio_background_colorpicker').on("mousedown", 'button[data-color]', function (e) {
            self._onColorChange($(e.currentTarget).data('value').replace('text-', 'bg-'), "background");
        });
        this.$('.o_custom_studio_font_colorpicker .o_custom_studio_color_palette').append(this._createPalette());
        this.$('.o_custom_studio_font_colorpicker').on("mousedown", 'button[data-color]', function (e) {
    ...
    _createPalette: function () {
        ...
        this._groupColors.forEach(function (color) {
            ...
            if (typeof color === 'string') {
                ...
                $row = $('<div class="o_custom_studio_color_row"/>');
    ...
    _onCustomColor: function (e) {
        ...
        colorpicker.on('colorpicker:saved', this, (ev) => {
            ...
            $button.attr('data-value', color);
            $button.css('background-color', color);
            $(e.target).closest('.dropdown-item').find('.o_custom_studio_custom_colors').append($button);
            $button.mousedown();
        });
        colorpicker.open();
    },
...
var TField = AbstractEditComponent.extend({
    name: 'tfield',
    template : 'custom_studio.ReportDirectiveTField',
    selector: '[t-field]',
    /**
     * @override
     */
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
            var $expr = self.$('.o_custom_studio_tfield_fieldexpression');
...
var TIf = AbstractEditComponent.extend({
    name: 'tif',
    template : 'custom_studio.ReportDirectiveTIf',
    selector: '',
    events: _.extend({}, AbstractEditComponent.prototype.events, {
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
            return self.fieldSelector['t-if'].appendTo(self.$('.o_custom_studio_tif_ifexpression'));
        });
    },
...
var TElse = AbstractEditComponent.extend({
    name: 'telse',
    template : 'custom_studio.ReportDirectiveTElse',
    selector: '[t-else]',
    ...
    start: function () {
        var self = this;
        return this._super.apply(this, arguments).then(function () {
            return self.fieldSelector['t-else'].appendTo(self.$('.o_custom_studio_telse_elseexpression'));
        });
    },
    /**
     * @override
...
var TEsc = AbstractEditComponent.extend({
    name: 'tesc',
    template : 'custom_studio.ReportDirectiveTEsc',
    selector: '[t-esc]',
    /**
     * @override
     */
    ...
    start: function () {
        var self = this;
        return this._super.apply(this, arguments).then(function () {
            return self.fieldSelector['t-esc'].appendTo(self.$('.o_custom_studio_tesc_escexpression'));
        });
    },
});
...
var TSet = AbstractEditComponent.extend({
    name: 'tset',
    template : 'custom_studio.ReportDirectiveTSet',
    selector: '[t-set]',
    /**
     * @override
     */
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
            return Promise.all([
                self.fieldSelector['t-set'].appendTo(self.$('.o_custom_studio_tset_setexpression')),
                self.fieldSelector['t-value'].appendTo(self.$('.o_custom_studio_tset_valueexpression'))
            ]);
        });
    },
});
...
var TForeach = AbstractEditComponent.extend({
    name: 'tforeach',
    template : 'custom_studio.ReportDirectiveTForeach',
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
            return Promise.all([
                self.fieldSelector['t-as'].appendTo(self.$('.o_custom_studio_tas_asexpression')),
                self.fieldSelector['t-foreach'].appendTo(self.$('.o_custom_studio_tforeach_foreachexpression'))
            ]);
        });
    },
});
...
var BlockTotal = AbstractEditComponent.extend({
    name: 'blockTotal',
    template : 'custom_studio.BlockTotal',
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
            return Promise.all([
                self.fieldSelector.total_currency_id.appendTo(self.$('.o_custom_studio_report_currency_id')),
                self.fieldSelector.total_amount_untaxed.appendTo(self.$('.o_custom_studio_report_amount_untaxed')),
                self.fieldSelector.total_amount_total.appendTo(self.$('.o_custom_studio_report_amount_total')),
                self.fieldSelector.total_amount_by_groups.appendTo(self.$('.o_custom_studio_report_amount_by_groups'))
            ]);
        });
    },
    /**
...
var Column = AbstractEditComponent.extend({
    name: 'column',
    template : 'custom_studio.ReportColumn',
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
            return Promise.all([
                self.fieldSelector.size.prependTo(self.$('.o_custom_studio_size')),
                self.fieldSelector.offset.prependTo(self.$('.o_custom_studio_offset'))
            ]);
        });
    },
    /**
...
var Text = AbstractEditComponent.extend({
    name: 'text',
    template : 'custom_studio.ReportText',
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments)
            .then(function () {
                return self.fieldSelector.text.appendTo(self.$('.o_custom_studio_text'));
            }).then(function () {
...
var Image = LayoutEditable.extend({
    name: 'image',
    template: 'custom_studio.ReportImage',
    selector: 'img',
    /**
     * @override
     */
    ...
    start: function() {
        var self = this;
        return this._super.apply(this, arguments).then(function () {
            return self.fieldSelector.src.appendTo(self.$('.o_custom_studio_source'));
        });
    },
});
...
var Groups = AbstractEditComponent.extend({
    name: 'groups',
    template: 'custom_studio.ReportGroups',
    insertAsLastChildOfPrevious: true,
    /**
     * @override
     */
    ...
    start: function() {
        var self = this;
        return this._super.apply(this, arguments).then(function () {
            return self.fieldSelector.groups.appendTo(self.$('.o_custom_studio_groups'));
        });
    },
});
...
var TOptions = AbstractEditComponent.extend( {
    name: 'toptions',
    template : 'custom_studio.ReportDirectiveTOptions',
    ...
    start: function () {
        ...
        if (this.widget) {
            this.$('.o_custom_studio_toption_widget select').val(this.widget.key);
    ...
    _updateWidgetOptions: function () {
        ...
        this.$('.o_custom_studio_toption_options').remove();
        if (!this.widget || !this.widget.options.length) {
            return;
        }
        var $options = $(qweb.render('custom_studio.ReportDirectiveTOptions.options', this));
        var defs = _.map(this.widget.options, function (option) {
            var $option = $options.find('.o_custom_studio_toption_option_' + self.widget.key + '_' + option.key);
