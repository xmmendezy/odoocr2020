odoo.define('custom_studio.NewModel', function (require) {
"use strict";
...
const NewModelItem = Widget.extend({
    xmlDependencies: ['/custom_studio/static/src/xml/new_model.xml'],
    template: 'custom_studio.NewModel',
    events: {
...
const NewModelDialog = CommonMenuDialog.extend({
    template: 'custom_studio.NewModel.Dialog',
    ...
    async start() {
        await this._super(...arguments);
        this.opened(() => {
            this.$modal.addClass('o_custom_studio_new_model_modal');
            // focus on input
