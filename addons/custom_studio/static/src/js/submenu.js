odoo.define('custom_studio.SubMenu', function (require) {
"use strict";
...
var SubMenu = Widget.extend({
    template: 'custom_studio.Menu',
    events: {
        'click .o_menu_sections > li': '_onMenu',
        'click .o_custom_studio_undo': '_onUndo',
        'click .o_custom_studio_redo': '_onRedo',
        'click .o_menu_sections .o_custom_studio_views_icons > a': '_onIcon',
    },
    /**
     * @constructor
     * @param {Widget} parent
    ...
    init: function (parent, action) {
        ...
        this.studio_actions = [{action: 'action_custom_studio_action_editor', title: 'Views'}];
    ...
    _renderBreadcrumb: function () {
        ...
        this.$('.o_custom_studio_breadcrumb')
            .empty()
            .append($breadcrumb);
    },
    /**
    ...
    _replaceAction: function (action, title, options) {
        if (options.viewType) {
            if (options.index > 1) {
                ...
                this.studio_actions = [
                    {action: 'action_custom_studio_action_editor', title: _t('Views')},
        ...
        if (action === 'action_custom_studio_action_editor') {
    ...
    _setActiveButtons() {
        ...
        const is_action_overview = cur_act.action === 'action_custom_studio_action_editor' && !cur_act.viewType;
        ...
        const isReportAction = currentAction.action === 'custom_studio.action_edit_report';
        // undo / redo button should be displayed only when editing view or a report
        this.$('.o_custom_studio_menu_undo_redo')
            .toggleClass('d-none', !currentAction.viewType && !isReportAction);
        this.$('.o_custom_studio_breadcrumb')
            .toggleClass('o_custom_studio_breadcrumb_report', isReportAction);
    },
    ...
    _onEditionModeEntered: function (viewType) {
        if (this.studio_actions.length === 1) {
            var bcOptions = {
                action: 'action_custom_studio_action_editor',
                viewType: viewType,
    ...
    _onIcon: function (ev) {
        ...
        return this._replaceAction('action_custom_studio_action_editor', view_name, {
            action: this.action,
            replace_last_action: true,
            viewType: view_name,
        });
    ...
    _onMenu: function (ev) {
        ...
        if (name === 'views') {
            this._replaceAction('action_custom_studio_action_editor', title, {
                action: this.action,
                studio_clear_studio_breadcrumbs: true,
            });
        } else if (_.contains(['automations', 'reports', 'acl', 'filters', 'translations'], name)) {
            var self = this;
            ajax.jsonRpc('/custom_studio/get_studio_action', 'call', {
                action_name: name,
    ...
    _onReportTemplateOpened: function (reportName) {
        ...
        var bcOptions = {
            action: 'custom_studio.action_edit_report',
            title: reportName,
        };
        this._addAction(bcOptions);
    },
    ...
    _onToggleUndo: function (display) {
        ...
        this.$('.o_custom_studio_undo').toggleClass('o_custom_studio_active', display);
    },
    /**
     * @private
     * @param {Boolean} display
    ...
    _onToggleRedo: function (display) {
        ...
        this.$('.o_custom_studio_redo').toggleClass('o_custom_studio_active', display);
    },
    /**
     * @private
    ...
    _onToggleSnackBar(type) {
        switch (type) {
            case 'saved':
                this.$('.o_custom_studio_snackbar_icon')
                    .removeClass('fa-spinner fa-pulse');
                this.$('.o_custom_studio_snackbar_icon')
                    .addClass('show fa fa-check');
                this.$('.o_custom_studio_snackbar_text')
                    .text(_t("Saved"));
                break;
            case 'saving':
                this.$('.o_custom_studio_snackbar_icon')
