odoo.define('custom_studio.ActionEditorActionTests', function (require) {
    "use strict";
    ...
    QUnit.module('Studio', {
        ...
        QUnit.test('add a gantt view', async function (assert) {
            ...
            var actionManager = await createActionManager({
                ...
                mockRPC: function (route, args) {
                    if (route === '/custom_studio/add_view_type') {
            ...
            await testUtils.dom.click(actionManager.$('.o_custom_studio_view_type[data-type="gantt"] .o_custom_studio_thumbnail'));

            assert.containsOnce($, '.o_custom_studio_new_view_modal',
                "there should be an opened dialog to select gantt attributes");
            assert.strictEqual($('.o_custom_studio_new_view_modal select[name="date_start"]').val(), 'start',
                "date start should be prefilled (mandatory)");
            assert.strictEqual($('.o_custom_studio_new_view_modal select[name="date_stop"]').val(), 'start',
        ...
        QUnit.test('disable the view from studio', async function (assert) {
            ...
            const actionManager = await createActionManager({
                ...
                async mockRPC(route) {
                    if (route === '/custom_studio/edit_action') {
                        return true;
